...
describe('Basic event tests', () => {
	...
	it('should attach basic click events', (done) => {
		...
		expect(calledFirstTest).toEqual(true);
		...
		expect(calledFirstTest).toEqual(false);
		expect(calledSecondTest).toEqual(true);
		...
		expect(calledFirstTest).toEqual(false);
		expect(calledSecondTest).toEqual(false);
		done();
	});
	...
	it('should update events', () => {
		...
		expect(container.firstChild.innerHTML).toEqual('Count 0');
		expect(data.count).toEqual(0);
		buttons.forEach((button) => button.click());
		expect(container.firstChild.innerHTML).toEqual('Count 1');
		expect(data.count).toEqual(1);
		buttons.forEach((button) => button.click());
		expect(container.firstChild.innerHTML).toEqual('Count 2');
	...
	it('should not trigger click at all if target is disabled', () => {
		...
		expect(container.firstChild.innerHTML).toEqual('<span>Count 0</span>');
		expect(data.count).toEqual(0);
		buttons.forEach((button) => button.click());
		expect(container.firstChild.innerHTML).toEqual('<span>Count 0</span>');
	});
	...
	it('should not leak memory when child changes', () => {
		...
		expect(container.innerHTML).toEqual('<p><div>2</div><div>2</div><div>2</div></p>');

		childrenArray.pop();
		// eslint-disable-next-line
		render(App(childrenArray), container);
		expect(container.innerHTML).toEqual('<p><div>2</div><div>2</div></p>');

		// eslint-disable-next-line
		render(AppTwo(), container);
		expect(container.innerHTML).toEqual('<p>2</p>');
	});
	...
	describe('Event Propagation', () => {
		...
		it('Should stop propagating Synthetic event to parentElement with synthetic event', (done) => {
			...
			setTimeout(function () {
				expect(eventHandlerSpy.callCount).toEqual(1);
				expect(eventHandlerSpy2.callCount).toEqual(0);
				done();
			}, 20);
		});
		...
		it('Should NOT stop propagating Synthetic event to parentElement with normal event', (done) => {
			...
			setTimeout(function () {
				expect(eventHandlerSpy.callCount).toEqual(1);
				expect(eventHandlerSpy2.callCount).toEqual(1);
				done();
			}, 20);
		});
	...
	describe('currentTarget', () => {
		it('Should have currentTarget', (done) => {
			function verifyCurrentTarget(event) {
				expect(event.currentTarget).toEqual(container.firstChild);
				done();
			}
		...
		it('Should work with deeply nested tree', (done) => {
			function verifyCurrentTarget(event) {
				expect(event.currentTarget).toEqual(container.querySelector('#test'));
				done();
			}
