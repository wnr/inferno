	...
	describe('Keys should always be unique within siblings', () => {
		...
		function verifyKeys(vNodes) {
			if (Array.isArray(vNodes)) {
				...
				expect(isDuplicate).toEqual(false, 'All siblings should have unique key');
		...
		it('Should handle nested arrays when keys are defined', () => {
			...
			expect(container.innerHTML).toEqual('<div><div>0 (0)</div><div>A (1)</div><div>B (2)</div><div>1 (3)</div><div>A (4)</div><div>B (5)</div><div>2 (6)</div></div>');
			...
			expect(container.innerHTML).toEqual('<div><div>0 (0)</div><div>B (2)</div><div>A (1)</div><div>1 (3)</div><div>B (5)</div><div>A (4)</div><div>2 (6)</div></div>');
		});
