...
describe('setState', () => {
	...
	it('should throw an error when setState is called in constructor', () => {
		...
		expect(render.bind(render, <TestComponent />, container)).toThrow(Error);
	});
	...
	it('callback should be fired after state has changed', (done) => {
		...
		class TestComponent extends Component<any, any> {
			...
			checkSetState() {
				...
				expect(value).toEqual('__NEWVALUE__');
				setTimeout(function() {
					done();
				}, 100);
			}
	...
	it('Should have new state in render when changing state during componentWillReceiveProps', (done) => {
		...
		class Child extends Component<any, any> {
			...
			callback() {
				expect(container.firstChild.firstChild.innerHTML).toEqual('bar2');
				done();
			}
		...
		expect(container.firstChild.firstChild.innerHTML).toEqual('bar');
	...
	it('Should have new state in render when changing state during componentWillMount and render only once', () => {
		...
		expect(container.firstChild.firstChild.innerHTML).toEqual('4');
	...
	it('setState must be sync like React if no state changes are pending', () => {
		...
		expect(container.firstChild.innerHTML).toEqual('b');
		doSomething('1');
		expect(container.firstChild.innerHTML).toEqual('1');
		doSomething('2');
		expect(container.firstChild.innerHTML).toEqual('2');
		doSomething('3');
		expect(container.firstChild.innerHTML).toEqual('3');
		doSomething('4');
		expect(container.firstChild.innerHTML).toEqual('4');
	});
